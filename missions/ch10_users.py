  ...
  └───────────────────────────────────────────────────────────────────┘
        ...
        quiz_questions    = [
            QuizQuestion(
                ...
                correct     = 'B',
                explanation = 'getent group GRUPPE zeigt alle Mitglieder. grep funktioniert auch, aber getent berücksichtigt auch LDAP/NIS.',
                xp_value    = 25,
            ),
            QuizQuestion(
                question    = "usermod -G sudo ghost — was ist die GEFAHR dieses Befehls?",
                options     = [
                    'A) Kein Problem — fügt ghost zur sudo-Gruppe hinzu',
                    'B) -G ERSETZT alle Gruppen von ghost (nur noch sudo!)',
                    'C) Fehler: -G existiert nicht, muss -g sein',
                    'D) ghost wird gesperrt',
                ],
                correct     = 'B',
                explanation = 'usermod -G = REPLACE alle Gruppen. usermod -aG = APPEND. Ohne -a verliert ghost alle anderen Gruppen!',
                xp_value    = 35,
            ),
            QuizQuestion(
                question    = "Welches Dateiformat verwendet /etc/shadow für Passwort-Hashes?",
                options     = [
                    'A) Klartext-Passwort',
                    'B) $algorithm$salt$hash (z.B. $6$ = SHA-512)',
                    'C) MD5 ohne Salt',
                    'D) Base64-kodiertes Passwort',
                ],
                correct     = 'B',
                explanation = '$1$=MD5, $5$=SHA-256, $6$=SHA-512. Format: $id$salt$hash. ! oder * = gesperrter Account. leeres Feld = kein Passwort.',
                xp_value    = 30,
            ),
            QuizQuestion(
                question    = "chage -l ghost zeigt 'Password expires: never'. Was bedeutet das?",
                options     = [
                    'A) Account ist abgelaufen',
                    'B) Passwort-Ablauf ist deaktiviert — Passwort läuft nie ab',
                    'C) chage wurde nicht korrekt konfiguriert',
                    'D) ghost hat sudo-Rechte',
                ],
                correct     = 'B',
                explanation = 'chage verwaltet Account-Aging. never = kein Ablaufdatum. chage -M 90 USER = Passwort läuft nach 90 Tagen ab.',
                xp_value    = 25,
            ),
            QuizQuestion(
                question    = "Was ist der korrekte Befehl um einen User-Account dauerhaft zu sperren (Login unmöglich)?",
                options     = [
                    'A) userdel ghost',
                    'B) passwd -l ghost (lock)',
                    'C) chmod 000 /home/ghost',
                    'D) rm /etc/passwd | grep ghost',
                ],
                correct     = 'B',
                explanation = 'passwd -l = lock: setzt ! vor Hash in /etc/shadow. Login unmöglich ohne Daten zu löschen. passwd -u = unlock.',
                xp_value    = 30,
            ),
        ],
        exam_tip     = (
