...
CHAPTER_20_MISSIONS: list[Mission] = [
    ...
    Mission(
        ...
        quiz_questions = [
            QuizQuestion(
                question = "Welche drei Befehle bilden den Standard-Firewall-Audit?",
                options  = [
                    "iptables -L, nft list ruleset, ss -tulpn",
                    "ps aux, netstat, nmap",
                    "journalctl, dmesg, syslog",
                    "ip addr, ip route, ip link",
                ],
                correct  = 0,
                explanation = "iptables -L (Regeln), nft list ruleset (nftables), ss -tulpn (lauschende Ports) = kompletter Netz-Audit.",
                xp_value = 30,
            ),
        ],
