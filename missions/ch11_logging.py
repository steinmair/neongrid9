  ...
  └───────────────────────────────────────────────────────────────────┘
        ...
        quiz_questions    = [
            QuizQuestion(
                ...
                correct     = 'B',
                explanation = 'journalctl -b 0 = aktueller Boot. -b -1 = vorletzter Boot. -b -2 = vor-vorletzter. Nützlich zur Post-Crash-Analyse.',
                xp_value    = 25,
            ),
            QuizQuestion(
                question    = "Was bedeutet Syslog-Severity '3' (err)?",
                options     = [
                    'A) Normaler Informationsstatus',
                    'B) Nur Debugging-Output',
                    'C) Fehlerbedingung — Dienst läuft aber mit Problemen',
                    'D) System ist nicht erreichbar',
                ],
                correct     = 'C',
                explanation = 'Syslog Severity: 0=emerg, 1=alert, 2=crit, 3=err, 4=warning, 5=notice, 6=info, 7=debug. 3=err = Fehlerbedingung, kein Absturz.',
                xp_value    = 30,
            ),
            QuizQuestion(
                question    = "Crontab-Zeile: '*/15 9-17 * * 1-5 /script.sh' — wann läuft das Skript?",
                options     = [
                    'A) Jede 15. Minute, jederzeit, nur Montag',
                    'B) Alle 15 Min zwischen 9 und 17 Uhr, nur Montag bis Freitag',
                    'C) Jeden 15. Tag, 9-17 Uhr, nur Wochentage',
                    'D) Alle 15 Stunden, 9-17 Tage, Montag bis Freitag',
                ],
                correct     = 'B',
                explanation = '*/15=alle 15 Min, 9-17=Stunden, *=jeder Tag, *=jeder Monat, 1-5=Mo-Fr. Cron-Felder: MIN STD TAG MON WOT.',
                xp_value    = 35,
            ),
            QuizQuestion(
                question    = "Welche rsyslog-Facility empfängt Auth-Nachrichten (sudo, login, su)?",
                options     = ['A) kern', 'B) auth / authpriv', 'C) daemon', 'D) user'],
                correct     = 'B',
                explanation = 'auth (und authpriv für sensitive Auth-Daten) empfängt Login, sudo, su. /var/log/auth.log auf Debian.',
                xp_value    = 25,
            ),
            QuizQuestion(
                question    = "logrotate.conf: 'rotate 7' bedeutet:",
                options     = [
                    'A) Rotation alle 7 Tage',
                    'B) 7 alte Log-Versionen werden aufbewahrt, älteste wird gelöscht',
                    'C) Maximale Log-Größe 7 MB',
                    'D) rotate läuft 7 Mal pro Tag',
                ],
                correct     = 'B',
                explanation = 'rotate N = N alte Versionen behalten. Nach N+1ter Rotation wird die älteste Datei gelöscht. weekly + rotate 7 = 7 Wochen History.',
                xp_value    = 30,
            ),
        ],
        exam_tip     = (
