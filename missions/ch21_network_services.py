...
CHAPTER_21_MISSIONS: list[Mission] = [
  ...
  ⚡ CHAOSWERK FACTION :: CHAPTER 21 BOSS ⚡""",
        ...
        quiz_questions = [
            QuizQuestion(
                question = "Ein NFS-Client kann einen Export nicht mounten. Welcher Befehl auf dem SERVER zeigt alle aktiven NFS-Exporte mit Clients?",
                options  = [
                    "exportfs -v",
                ...
                correct  = 0,
                explanation = "exportfs -v listet aktive Exporte mit Optionen und erlaubten Hosts. showmount -e zeigt RPC-Exports, aber exportfs -v ist die LPIC-1-Referenzantwort für Server-seitige Diagnose.",
                xp_value = 25,
            ),
            QuizQuestion(
                question = "Was prüft 'testparm' auf einem Samba-Server?",
                options  = [
                    "Syntaxfehler in /etc/samba/smb.conf",
                    "Ob ein SMB-Client erfolgreich verbinden kann",
                    "Die Samba-Passwortdatenbank auf Konsistenz",
                    "Netzwerkerreichbarkeit des Samba-Servers",
                ],
                correct  = 0,
                explanation = "testparm liest smb.conf und meldet Syntaxfehler oder unbekannte Parameter — analog zu 'nginx -t'. Es testet keine Konnektivität oder Passwörter.",
                xp_value = 25,
            ),
            QuizQuestion(
                question = "Ein DHCP-Server vergibt keine Adressen mehr. In welchem File speichert ISC DHCP die aktuellen Lease-Informationen?",
                options  = [
                ...
                correct  = 0,
                explanation = "dhcpd.leases enthält alle aktiven und abgelaufenen Leases. dhcpd.conf ist die Konfiguration (Subnetze, Bereiche). Das Lease-File wächst kontinuierlich und wird periodisch bereinigt.",
                xp_value = 30,
            ),
            QuizQuestion(
                question = "Welcher DNS-Record-Typ mappt einen Hostnamen auf eine IPv6-Adresse?",
                options  = [
                    "AAAA",
                    "A6",
                    "PTR",
                ...
                correct  = 0,
                explanation = "AAAA (quad-A) ist der DNS-Record für IPv6-Adressen. A ist für IPv4. A6 ist veraltet und nie standardisiert. PTR ist Reverse-DNS. CNAME ist ein Alias.",
                xp_value = 30,
            ),
            QuizQuestion(
                question = "Was ist die Aufgabe von /etc/nsswitch.conf in Bezug auf Namensauflösung?",
                options  = [
                    "Legt die Reihenfolge fest, in der Quellen (files, dns, ldap) für Lookups verwendet werden",
                    "Konfiguriert die DNS-Server-IP-Adressen für den Resolver",
                    "Definiert lokale Hostnamen-zu-IP-Mappings",
                    "Steuert, welche Dienste beim Start automatisch laufen",
                ],
                correct  = 0,
                explanation = "nsswitch.conf bestimmt die Lookup-Reihenfolge für hosts:, passwd:, group: etc. — z.B. 'hosts: files dns' bedeutet: erst /etc/hosts, dann DNS. /etc/resolv.conf hat die DNS-Server.",
                xp_value = 35,
            ),
        ],
