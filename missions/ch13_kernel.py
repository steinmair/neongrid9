  ...
  ⚡ CHAOSWERK FACTION :: CHAPTER 13 BOSS ⚡""",
        ...
        quiz_questions    = [
            QuizQuestion(
                ...
                correct     = 'B',
                explanation = 'modprobe = loads module + dependencies. insmod = low-level, kein Dependency-Management. modprobe liest modules.dep.',
                xp_value    = 25,
            ),
            QuizQuestion(
                question    = "sysctl -w net.ipv4.ip_forward=1 — was ist das PROBLEM dieser Änderung?",
                options     = [
                    'A) Syntax falsch — sollte sysctl net.ipv4.ip_forward 1 heißen',
                    'B) Nur temporär — nach Reboot auf 0 zurück gesetzt',
                    'C) Aktiviert Firewall — blockiert alle Pakete',
                    'D) Kein Problem — die Änderung ist permanent',
                ],
                correct     = 'B',
                explanation = 'sysctl -w = write, aber nur bis zum nächsten Reboot. Für permanente Änderung: /etc/sysctl.conf bearbeiten + sysctl -p ausführen.',
                xp_value    = 30,
            ),
            QuizQuestion(
                question    = "Ein Modul soll nie geladen werden (blacklist). Wo wird das konfiguriert?",
                options     = [
                    'A) /etc/modules (Blacklist-Zeile hinzufügen)',
                    'B) /etc/modprobe.d/blacklist.conf (blacklist MODULNAME)',
                    'C) /proc/modules (Modul-Eintrag löschen)',
                    'D) modprobe --disable MODULNAME',
                ],
                correct     = 'B',
                explanation = '/etc/modprobe.d/*.conf enthält blacklist-Einträge. Format: blacklist MODULNAME. Wirkt beim nächsten Boot / nach modprobe-Aufruf.',
                xp_value    = 30,
            ),
            QuizQuestion(
                question    = "uname -r gibt '6.1.0-21-amd64' zurück. Was bedeutet '6.1.0'?",
                options     = [
                    'A) Kernel API Version',
                    'B) Major.Minor.Patch (6=major, 1=minor, 0=patch)',
                    'C) Buildnummer',
                    'D) Release-Datum',
                ],
                correct     = 'B',
                explanation = 'Kernel-Version: MAJOR.MINOR.PATCH. 6=major-version, 1=minor (Feature), 0=patch (Bugfix). -21=ABI-bump, amd64=Architektur.',
                xp_value    = 25,
            ),
            QuizQuestion(
                question    = "Was zeigt 'dmesg -l err,crit' an?",
                options     = [
                    'A) Kernel-Nachrichten mit Level err und crit (Severity 3 und 2)',
                    'B) Alle Nachrichten außer err und crit',
                    'C) Nur die letzten 3 Nachrichten',
                    'D) Fehler: dmesg hat kein -l Flag',
                ],
                correct     = 'A',
                explanation = 'dmesg -l LEVEL filtert nach Log-Level. err=Level3, crit=Level2. dmesg -T zeigt Human-Readable-Timestamps.',
                xp_value    = 30,
            ),
        ],
        exam_tip     = (
